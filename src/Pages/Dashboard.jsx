...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setStatus('❌ Failed to load dashboard data. Is the backend running at http://localhost:8000?');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRecalculate = async () => {
    try {
      ...
      setStatus('❌ Error running allocation. Check backend logs.');
    } finally {
      setRunning(false);
    }
  };
  ...
  // Show error state if backend is unreachable and no data loaded
  if (!data) {
    return (
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold text-blue-950">📊 Drought Monitoring Dashboard</h1>
          <p className="text-gray-500 mt-1 text-sm">Real-time Water Stress Monitoring · Vidarbha, Maharashtra</p>
        </div>
        <div className="bg-red-50 border border-red-200 rounded-2xl p-8 text-center">
          <p className="text-red-700 font-semibold text-lg mb-2">
            {status || '❌ Unable to load dashboard data'}
          </p>
          ...
          <button
            onClick={fetchData}
            className="mt-4 px-5 py-2 bg-red-600 text-white rounded-xl font-semibold hover:bg-red-700 transition"
          >
            🔄 Retry
          </button>
        </div>
